...
type CSSPropertiesWithCustom = CSSProperties & {
    ...
    >
      ...
      <svg x={x} y={y} className="overflow-visible">
        {squares.map(({ pos: [squareX, squareY], id: squareId }, index) => (
          // Predetermined motion stays in CSS (see globals.css
          // `grid-square`). The key is stable so the animation never
          // restarts; the square hops to a new cell while it is invisible,
          // at each iteration boundary.
          <rect
            key={squareId}
            className="opacity-0 motion-safe:animate-grid-square"
            style={
              {
                "--grid-square-duration": `${duration * 2 + repeatDelay}s`,
